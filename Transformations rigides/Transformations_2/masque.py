...
def main(img_path):
    gray = niv_de_gris(img_path)
    tab_normal = normalise_fun(gray)
    masque = masque_fun(tab_normal)
    
    cv2.imwrite('Transformations rigides\\Transformations_2\\tests\\masque.jpg', masque)
    
    plt.figure(figsize=(10, 10))
    plt.imshow(masque, cmap='gray')# inversant les canaux BGR → RGB pour matplotlib ??
    plt.title("minutiae detecteees")
    plt.axis("off")
    plt.show()
    
    return('Transformations rigides\\Transformations_2\\tests\\masque.jpg')